...
* `theme` contains a valid value (`"Tags"`, `"Virtual"`, `"Unknown"`, `"Physical"`, or `"other: \w{2,}"`).
...
* `typeOfTopoExpression` is one of the valid enumeration or pattern values (`"Contains"`, `"Overlaps"`, `"Equals"`, `"Within"`, `"Crosses"`, `"Other"`, or `"other: \w{2,}"`).
...
* The order of `connectedLayers`, `connectedCells`, and `connectedNodes` corresponds pairwise to the same indices.
* A unique `gml:id` attribute is present.
--
...
* `locomotionType` contains a valid value (`"Walking"`, `"Flying"`, `"Rolling"`, `"Unspecified"`, or `"other: \w{2,}"`).
* A unique `gml:id` attribute is present.
--
...
* `function` contains a valid value (`"Administration"`, `"Business, trade"`, `"Education, training"`, `"Recreation"`, `"Laboratory"`, `"Storage"`, `"Security"`, or `"other: \w{2,}"`).
* A unique `gml:id` attribute is present.
--
...
--
* All validation criteria for <<conf-navigablespace,`/conf/navi/navigablespace`>> are satisfied.
* `function` contains a valid value (`"ConnectionSpace"`, `"AnchorSpace"`, or `"other: \w{2,}"`).
* `type` contains a valid value (`"Door"`, `"Window"`, or `"other: \w{2,}"`).
* A unique `gml:id` attribute is present.
--
...
* `NavigableBoundaryFunction` contains a valid value (`"ConnectionBoundary"`, `"AncorBoundary"` / `"AnchorBoundary"`, or `"other: \w{2,}"`).
