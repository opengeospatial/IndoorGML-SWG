...
The JavaScript Object Notation (JSON) encoding of IndoorGML 2.0 (IndoorJSON) implements the conceptual classes of <<ogc22-045r5,IndoorGML 2.0 Part 1>> as JSON objects.
The conceptual model remains normative for semantics, while this document defines the JSON representation.
...
[[fig-core-indoorgml]]
.IndoorGML 2.0 core module structure UML
image::../figures/core-module-uml.png[align="center"]

The IndoorJSON Core schema follows <<fig-core-indoorgml>>.
Each class is defined individually, and depending on its relationships, it can hold other classes' IDs as property values or array values.
For example, <<section-indoorfeatures,`*IndoorFeatures*`>> have `layers` property for <<section-thematiclayer,`*ThematicLayer*`>> and `layerConnections` property for <<section-interlayerconnection,`*InterLayerConnection*`>>.

IndoorJSON Navigation preserve the IndoorGML 2.0 navigation conceptual model structure, as shown in the below figure.

[[fig-navi-indoorgml]]
.IndoorGML 2.0 navigation module structure UML
image::../figures/navi-module-uml.png[align="center"]

The IndoorJSON Navigation schema follows <<fig-navi-indoorgml>>.
Each class is defined individually.
Depending on the inheritance relationship, a child class inherits the properties of its parent class.

==== JSON-FG-compatible geometry

IndoorJSON use <<ogc21-045r1,JSON-FG>>-compatible geometry objects for the geometry representation.
This is necessary because indoor spaces require 3D geometry, including solid geometry.
The current IndoorJSON core schema references JSON-FG geometry schemas for `geometry2D` and `geometry3D`, including *Polygon* and *Polyhedron* for `CellSpace`.
