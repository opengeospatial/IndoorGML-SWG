...
A cellular space _S_ of thematic layer _T_, noted _S^T^_ is defined as follows:

_S^T^_= {_c~1~_, _c~2~_, …, _c~n~_}

where _c~i~_ is _i_^th^ cell. Every cell in a cellular space can have the following properties:
...
* mode: walking, driving, flying (see <<fig-subdivision-and-aggregation>>.c and <<fig-subdivision-and-aggregation>>.d)
...
<<fig-cellular-space-visitor>> represents a thematic layer 'Visitors', which contains all cells, which are accessible to visitors in a university faculty (<<alattas2017supporting,Alattas et al. 2017>>).
